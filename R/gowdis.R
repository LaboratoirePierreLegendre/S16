`gowdis` <- function(x, w, asym.bin = NULL, ord = c("podani", "metric", "classic") )
{
  s16 <- function(d, k) 2 * (k + 1 - d) / (2 * k + 2 + d * k)
  ...
  res <- .C("gowdis",
            as.double(x), as.double(w), as.integer(type), as.integer(n), as.integer(p), as.double(range2),
            as.integer(pod), as.double(Timax), as.double(Timin), res = double(n*(n-1)/2), NAOK = T,
            PACKAGE = "S16")$res
